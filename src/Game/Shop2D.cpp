...
void Shop2D::Init()
{
	...
	D3DXMatrixTranslation(&N_OpenMat, 1280.0f / 2.0f, 720.0f / 2.0f, 0.0f);
	BuyTex = RESOURCE_MNG.GetTexture("Shop/Buy.png");
	D3DXMatrixTranslation(&BuyMat, 1280.0f / 2.0f, 720.0f / 2.0f, 0.0f);

	SoldOutTex = RESOURCE_MNG.GetTexture("Shop/SoldOut.png");
	for (int i = 0; i < LISTNUMBER; i++) {
		for (int j = 0; j < TAB; j++) {
			D3DXMatrixTranslation(&SoldOutMat[j][i], 470.0f, 183.0f + (60.5f * i), 0.0f);
		}
	}
...
void Shop2D::Draw2D()
{
	...
	RECT rcN_OpenBuy = { 0,0,600,100 };
	SPRITE->SetTransform(&N_OpenMat);
	if (NotOpenFlg) {
		SPRITE->Draw(*N_OpenTex, &rcN_OpenBuy, &D3DXVECTOR3(300.0f, 50.0f, 0.0f), NULL, D3DCOLOR_ARGB(255, 255, 255, 255));
	}
	RECT rcBuy = { 0,0,600,100 };
	SPRITE->SetTransform(&BuyMat);
	if (BuyFlg) {
		SPRITE->Draw(*BuyTex, &rcCan_tBuy, &D3DXVECTOR3(300.0f, 50.0f, 0.0f), NULL, D3DCOLOR_ARGB(255, 255, 255, 255));
	}
