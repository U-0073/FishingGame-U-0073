...
void Result::Init() 
{
	...
	if (ScoreData.Success) {
		...
		vTex = RESOURCE_MNG.GetTexture("escape.png", 336, 200, NULL);
		mPos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		D3DXMatrixTranslation(&mMat, 0.0f, 0.0f, 0.0f);
		mMat.SetScale(2.0f, 2.0f, 2.0f);
		vBackTex = RESOURCE_MNG.GetTexture("SeaBack001.png", 1280, 720, NULL);
		mBackPos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		D3DXMatrixTranslation(&mBackMat, 0.0f, 0.0f, 0.0f);
...
void Result::Update()
{
	if (ScoreData.Success) {
		...
		D3DXMatrixTranslation(&mBackMat, mBackPos.x, mBackPos.y, mBackPos.z);
	}
}
