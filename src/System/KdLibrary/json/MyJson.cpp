...
{
	if (Json&&(*Json)[Tag].is_string() && (*Json)[Tag].string_value() == Value)return true;
	 else return false;
}

 bool MyJson::checkValue(const std::shared_ptr<json11::Json> Json, const std::string& Tag, const int& Value)
{
	if (Json && (*Json)[Tag].is_number() && (*Json)[Tag].int_value() == Value) return true;
...
{
	if (Json && (*Json)[Tag].is_number() && (*Json)[Tag].number_value() == Value)return true;
...
{
	if (Json && (*Json)[Tag].is_bool() && (*Json)[Tag].bool_value() == Value)return true;
